...
Use as a library:  from make_sample_data import make_sample; df = make_sample()
Or run as a script: python make_sample_data.py   (writes sample_data/loan_approvals_synthetic.csv)
"""
import os
...
def make_sample(n: int = 5000, seed: int = 42) -> pd.DataFrame:
...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(here, "sample_data"), exist_ok=True)
    out = os.path.join(here, "sample_data", "loan_approvals_synthetic.csv")
    data = make_sample()
    data.to_csv(out, index=False)
    print("Saved", out, data.shape)
    print(data.groupby("gender")["loan_approved"].mean().round(3))
